		...
		if (existsInMap(n1) && existsInMap(n2)){
			...
			if ((existsInMap(n1) && !existsInMap(n2)) || (existsInMap(n2) && !existsInMap(n1))){
				...
				if (!this.thetaSubsumes(absentNode, presentNode)){
					this.map.put(absentKey, presentNode);	// Change to binary tree in this condition!
				}
				return presentNode;
			}
			else {
...
//		System.out.println("BASE-VARIABLE");
		if (existsInMap(base) && variableExistsInMap(variable)){
			...
			if (existsInMap(base)){
				...
				this.map.put(base.getIdentifier(), variable);  // Change to binary tree in this condition
				return variable;
			}
		}
	}
	...
	public VariableNode variableVariableGeneralization(VariableNode v1, VariableNode v2){ 
		// 		Revise this whole method for binary tree rebuild
...
//	=========== MAIN METHOD FOR TESTING ============
	...
	public static void main(String[] args){
		GeneralizationTree m = new GeneralizationTree();
		Entity e = new Entity();
...
//		System.out.println(m.getGeneralization(v1, v2));
		m.displayMap();
		int i = 0;
		VariableNode var = m.getGeneralization(n1, n2);
		for (String key : m.map.keySet()){
			if (m.map.get(key).equals(var)) i++;
		}
		System.out.println(m.map.values());
